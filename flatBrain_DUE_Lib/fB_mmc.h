...
const uint8_t STA_NOINIT = 0x01; 	/* Drive not initialized */
const uint8_t STA_NODISK = 0x02; 	/* No medium in the drive */
const uint8_t STA_PROTECT = 0x04; 	/* Write protected */
...
namespace mmc
{
	uint8_t initialize(uint8_t speed);
	...
	*/
	uint8_t readSector(uint8_t *buffer, unsigned long sector);
	...
	* are made.
	*/
	uint8_t writeSector(const uint8_t *buffer, uint32_t sector);
