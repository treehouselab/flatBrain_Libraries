...
uint8_t fB_Menu::dPlaces(double value, uint8_t size) {  // returns number of visible decimal places in double if format is "size" chars and point is one character width.
	int j = 0;
	double dint,test;

	while(true) {
		test = fabs(value/pow(10.0,(double)j));
		if(test < 1) break;
		j++;
	}
	return (size - ( j + 1));
...
char *fB_Menu::d2str(double value, uint8_t size, char* buffer) { //formats double to string of size length, strips leading zero
	...
	char c;
	truncRound(value, _PRECISION);
	places = dPlaces(value,size);
	//dbug(F("d2str1 v:%f  p:%d"),value,min(places,_PRECISION));
	doubleToStr(value,min(places,_PRECISION),buffer);
	return buffer;
}
...
void fB_Menu::jumpPage(uint16_t tag) {
	fB_Tag *pT;
	pT = Tag(tag);
	//dbug(F("Jump from %P to tag:%d , title:%P, flags:0x%x"),curr.pP->title,pT->tag,pT->title,pT->flag16);
	//dbug(F("Jump from %P ,tag:%d ,cfY:%d TO %P  tag:%d "),curr.pP->title,curr.pageTag,curr.farY,pT->title,pT->tag);
...
void fB_Menu::jumpPage(fB_Tag *pT) {
	if(!pT ||!( pT->flag16 & _PAGE)) return;
	//dbug(F("Jump$ from %P to tag:%d , title:%P  cpT:%d"),curr.pP->title,pT->tag,pT->title,curr.pageTag);
	if(curr.pageTag) clearPage();
	showPage(pT->tag);
...
void fB_Menu:: clearPage(uint8_t  full) {
	//dbug(F("M CP %P ct:%d rc:%d, cfY:%d"),curr.pP->title, curr.pageTag,curr.getRowCount(),curr.farY);
...
//dbug(F("NX2 %P,  f16:%x , a:%L"),pT->title, pT->flag16,pT->getAction()) ;
		if( i > rowDex) {
			deselectRow();
			rowDex = i;
			selectRow();
...
void fB_Menu:: showPage(uint16_t tag, uint8_t pageOption) {
	...
	if(pageOption != REFRESHPAGE) {
		//Tag(HEADER)->title = curr.pP->title; 
		curr.deselectRow();
		curr.rowDex = 0;		
		curr.setCurrPage(tag);
		ptHEADER->title = curr.pP->title; 
...
void fB_Tag::showRow(int  rowIndex, uint8_t  option) {  //when option == REFRESH, only right col data is updated
	...
	if(option != REFRESHPAGE) {
		if(!(flag16 & _TTITLE)) {
			if(alias) pTitleText = alias;
			else pTitleText = title; 
		}
		else {
...
//dbug(F("mPPC mode:%d"),mode);
//dbug(F("mPPC %P  ,m:%d "),pT->title,pT->getMode());
		...
		Tag(PNPIN)->text= pT->title;
		Tag(PNPIN)->tTag =  (uint8_t)i; //  fTag avail to store index
		Tag(PNTOG)->iVal= LOW;
		Tag(PNADC)->iVal=0;
		
		//Tag(PNCRD)->Ptext= Card(pT->getCtag())->title;
		if((rowSide >> 7) == _L) side = 'L';
		else side = 'R';
		sprintf(rowBuffer,"%s %d%c",Card(pT->getCtag())->title,rowSide & 0x1F , side);
		Tag(PNROW)->text = rowBuffer;
		if(pT->getMode() == _DIGITAL) {
			...
			Tag(PNBIA)->dVal->value = pT->dVal->bias;
			Tag(PNOFF)->dVal->value = pT->dVal->offset;
			Tag(PNINC)->dVal->value = 0.01;
			if(pT->getOnVal() == _PGATE) {
	//dbug(F("PPC %P ,  F16:0x%x, fac:%f"),pT->title,pT->flag16,pT->dVal->bias);
	...
	switch(tag) {
		...
		case PNPIN: 
			...
			if(tagRay[Tag(PNPIN)->tTag].getMode() == _DIGITAL) menu.refreshRow(PNTOG);
				...
				menu.refreshRow(PNBIA);
				menu.refreshRow(PNOFF);
				menu.refreshRow(PNINC);
			}
			curr.selectRow();
			return 1;
		case PNGAT:
		...
		case PNBIA:
			pT = &tagRay[ Tag(PNPIN)->tTag];
			dVal->value += Tag(PNINC)->dVal->value;
			pT->dVal->bias = dVal->value;
			menu.refreshRow();
			return 1;
		case PNOFF:
			pT = &tagRay[ Tag(PNPIN)->tTag];
			dVal->value += Tag(PNINC)->dVal->value;
			pT->dVal->offset = dVal->value;
			menu.refreshRow();
			return 1;
		case CLKSET:
		...
		case _EAUTO:	if(iVal == HIGH) iVal = LOW;
					else iVal = HIGH;
					menu.refreshRow();
					rec.EEwriteEAUTO();
					menu.selectHeader();
...
*/
/*				case TROW:
					if(!Tag(TLOG)->text)  Tag(TLOG)->text = gRay[G_NOLOG] ;
					Tag(TSET)->tTag = pT->tag;
					Tag(TFAC)->tTag = pT->tag;
					Tag(TFAC)->iVal = pT->bias;
					Tag(TFAC)->iVal = pT->bias;
					...
					switch(pT->format) {
						...
						if(!(pT->rFlags & TINPUT))  {
							...
							Tag(TVAL)->type = _NOACT;
								Tag(TOPR)->text =gRay[G_AMP];
								if(!pT->bias) pT->bias = 1;
								Tag(TINC)->format = _FLOAT2;
 					    //}
					...
					break;
				case TARB:	if (!strcmp(text,menu.biasStr)) {
								text = menu.ampStr;
								Tag(TFAC)->iVal = 1;
							}
							else {
				...
				case TSET:	
												if(pT->rFlags & TINPUT)  {
								if(!strcmp(Tag(TOPR)->text,menu.biasStr))  {
