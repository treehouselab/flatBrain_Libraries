...
void defineUser() {
	...
	//////////////////////////////////////////
 	...
 	//////////// DEFINE CARDS AND PINS
		...
		definePin(_TALLED,YCRD,17,_L,_OUTPUT,HIGH);
	...
	//////////////////////////////////////////
   	...
   	definePage(VSTATUS,HOME);
		...
		defineRow(V0, _FLOAT1 | _LOADEE );
		defineRow(V1, _FLOAT1 | _UPDATE | _LOADEE);
		defineRow(V2, _FLOAT1 | _UPDATE | _LOADEE);
		defineRow(V3, _FLOAT1 | _UPDATE | _LOADEE);
		//defineRow(CZ,  _FLOAT2 );
		defineRow(CX,  _FLOAT1 | _UPDATE  | _LOADEE);
		defineRow(CL,  _FLOAT1 |  _UPDATE | _LOADEE);
...
*/
	...
	double posOnly(fB_Tag* pT,uint16_t intVal) { return max(intVal * pT->dVal->bias + pT->dVal->offset,0); }

	double sysAmps(fB_Tag* pT,uint16_t intVal) { return max((intVal - 512) * pT->dVal->bias + pT->dVal->offset,0); }

	double loadAmps(fB_Tag* pT,uint16_t intVal) {
		double load;
		load = (double)intVal * pT->dVal->bias + pT->dVal->offset;
		if(intVal < 50 || fabs(load) < 1) return 0;
		return load;
		//return  sign * ( fabs((intVal - 534 )* pT->dVal->bias) + pT->dVal->offset);
	}
