...
uint8_t mmc::initialize(uint8_t speed) {
  ...
  PORTB |=  _BV(SCK)   | _BV(MISO); // set SCK, pullup on MISO
  DDRB  |=  _BV(SCK)   | _BV(MOSI); // set SCK/MOSI as output
  DDRB  &= ~_BV(MISO); 
// set MISO as input
...
int mmc::sendCommand(const uint8_t  command, const uint32_t parameter, const uint8_t  deselect) {
  union {
    unsigned long l;
    unsigned char c[4];
  } 
...
uint8_t mmc::readSector(uint8_t *buffer, uint32_t sector)
{
...
uint8_t mmc::writeSector(const uint8_t *buffer, uint32_t sector)
{
