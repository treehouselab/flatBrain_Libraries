...
void fB_Record::createTagDefLog() {
	...
	if(fat.openFile(filename,FILEMODE_TEXT_WRITE)==NO_ERROR) { 
		...
		for(int i=0;i<tagCount;i++){
			...
			sprintf(buffer,P("%s,%d,%s, %s,%s, %d,%d,%d,%d"),title,pT->tag ,base,datastr	,menu.doubleToStr(pT->dVal->bias,3,datastr),menu.doubleToStr(pT->dVal->offset,3,datastr),
				1 && pT->flag16 & _LOADEE
//					,1 && pT->flag16 & 0x04
//					,1 && pT->flag16 & 0x08
//					,1 && pT->flag16 & 0x10
...
void fB_Record::EEclearTags(uint16_t offTags, uint16_t base) {
	// offTags is the number of tags in the front of the section NOT to clear
		...
		ee.setBlock(base+(offTags*32),'\0',32 * (count +1));  // leave blank of 32 bytes for EOF
}
...
void fB_Record::EEwriteTags(uint16_t base) {
	...
	for(int i=0;i<tagCount;i++){
		...
		if(!(pT->flag16 & _DUBL)){	
			...
			ee.writeBlock(addr+16,data,4);
			data = (uint8_t *)&(pT->dVal->offset);
			ee.writeBlock(addr+20,data,4);
		}
		j++;
	}	
	//ee.dump(0,128);
	...
	for(int i=0;i<tagCount;i++){
		...
		if(!strcmp(title,(char *) buffer)) {
			...
			if(pT->flag16 & _DUBL) {
				...
				ee.readBlock(addr+16,buffer,4);
				bufTag.dVal->bias = *(double*)buffer;
				ee.readBlock(addr+20,buffer,4);
				bufTag.dVal->offset = *(double*)buffer;
	...
	for(int i=0;i<logTagCount; i++) {
		...
		for(int j=0;j<tagCount;j++){
			...
			if(!strcmp(title,(char *) buffer)) {
				...
				ee.writeBlock(addr+16,NULL,4);
				ee.writeBlock(addr+20,NULL,4);
			}
		}
	}
}
	...
	for(int i=0;i<tagCount;i++){
		...
		if(!strcmp(title,(char *) buffer)) {
			if(pT->flag16 & _DUBL) {
				...
				ee.readBlock(addr+16,buffer,4);
				pT->dVal->bias = *(double*)buffer;
				ee.readBlock(addr+20,buffer,4);
				pT->dVal->offset = *(double*)buffer;
