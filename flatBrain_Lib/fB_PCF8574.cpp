...
void PCF8574::i2cSend(){
	...
	i2c.write((uint8_t )PCFaddress, (uint8_t )PCFPORTA);
}
...
void PCF8574::i2cRead(){
  uint8_t  value = 0x00;
  if(!i2c.read((uint8_t )PCFaddress, (uint8_t )1)) PCFPORTA = (int)i2c.receive();
  else PCFPORTA = (int)value; //error condition
}

int PCF8574::i2cRead(uint8_t  value){
  if(!i2c.read((uint8_t )PCFaddress, (uint8_t )1)) value = (int)i2c.receive();
  return value;
}
